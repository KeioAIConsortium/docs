...
```
ssh -p 2221 -L 8000:jupyterhub-singleuser-instance-[ユーザ名]:8000 [ユーザ名]@[ゲートウェイサーバ].ai.hc.keio.ac.jp
```

この時、ユーザ名とゲートウェイサーバは[アクセス手順](##アクセス手順)と同様に設定します。すると、SSHが接続されている限り`locahost:8000`へのアクセスは透過的にJupyter Notebokインスタンスの8000番ポートに転送されるようになります。

### JupyterHubインスタンスへのSSHアクセスの仕方

JupyterHubインスタンスに直接SSH接続をしたい場合は、公開鍵認証を使用することでアクセスできるようになります。

事前の準備として

- SSHキーペアの生成
- 接続元の端末での`ssh-agent`の起動，SSHキーペアの登録
- SSHの公開鍵をJupyterHubインスタンスに登録
  - `~/.ssh/autorized_keys`に書き込む，`ssh-import-id-gh [GitHubユーザ名]`でインポートする等
- (optional) SSHの公開鍵をゲートウェイサーバに登録
